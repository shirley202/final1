...
def pdf_to_html(request):
    ...
    if selected_carrera_nombre:
        try:
            selected_carrera = Carrera.objects.get(nombre__iexact=selected_carrera_nombre)
            pdfs = PDF.objects.filter(id_carrera=selected_carrera)
...
def get_materiasf(request, codcarrera):
    # Intentar obtener la carrera basada en el nombre
    try:
        carrera = Carrera.objects.get(nombre__iexact=codcarrera)
        # Filtrar PDFs usando el id de la carrera
        materias = list(PDF.objects.filter(id_carrera=carrera.id).values())
        if materias:
            data = {'message': "Success", 'materias': materias}
        else:
            data = {'message': "Not Found"}
    except Carrera.DoesNotExist:
        data = {'message': "Carrera Not Found"}
